...
class PlayerListFeatureListener(plugin: TitleManagerPlugin, coroutineScopeManager: CoroutineScopeManager) : TitleManagerListener(plugin, coroutineScopeManager) {
    ...
    init {
        registerEventExecutor<PlayerJoinEvent> { event ->
            playerContextManager.getContext(event.player).setConfigPlayerList()
        }
