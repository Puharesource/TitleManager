...
class ClampedColorGradientAnimation<TContext>(private val timing: Timing?, private val gradient: GradientConfiguration, private val separator: String, private val text: String) : Animation<TContext, String> {
    private val regularGradient: List<String> by lazy { gradient.getColors(false, text.length + 1) }
    private val radialGradient: List<String> by lazy { gradient.getColors(true, text.length + 1) }

    override fun singleIterationFlowWithTimings(context: TContext, isInfinite: Boolean): Flow<TimedItem<String>> = flow {
        val colors = if (isInfinite) radialGradient else regularGradient
