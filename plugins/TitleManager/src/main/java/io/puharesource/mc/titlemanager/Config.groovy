...
class Config {
    ...
    void reload() {
        ...
        for (String str : animationConfigFile.getConfig().getKeys(false)) {
            ...
            for (String frame : section.getStringList("frames")) {
                ...
                int stay = -1;
                int fadeOut = -1;
                frame = ChatColor.translateAlternateColorCodes('&'.toCharacter(), frame);
    ...
    //Animations
    FrameSequence getAnimation(String animation) { animations.get(animation.toUpperCase().trim()) }
    Map<String, FrameSequence> getAnimations() { animations }
