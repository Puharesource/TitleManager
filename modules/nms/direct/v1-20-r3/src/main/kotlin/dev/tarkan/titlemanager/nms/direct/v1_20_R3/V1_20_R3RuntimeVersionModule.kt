...
class V1_20_R3RuntimeVersionModule(
    ...
    override fun createSidebar(player: Player): RuntimeSidebar {
        ...
        return V1_20_R3RuntimeSidebar(player, sendPacket = { packet -> connection.send(packet) })
    }
    ...
    private fun Player.packetSinkOrNull(): V1_20_R3RuntimePacketSink? {
        val connection = connectionOrNull() ?: return null

        return V1_20_R3RuntimePacketSink { packet -> connection.send(packet) }
    }

}

internal class V1_20_R3RuntimePacketSink(
...
internal class V1_20_R3RuntimeSidebar(
    ...
    override fun set(index: Int, value: String) {
        ...
        sendPacket(ClientboundSetScorePacket(entryName, objective.name, RuntimeTextConstants.sidebarScore(index), null, null))
    }

    override fun remove(index: Int) {
        val line = lines.remove(index) ?: return

        sendPacket(ClientboundResetScorePacket(line.entryName, objective.name))
