...
class V1_20_R3RuntimeVersionModuleFactoryTest {
    @Test
    ...
    fun `factory public surface does not reference server internals`() {
        val publicTypes = buildList {
            V1_20_R3RuntimeVersionModuleFactory::class.java.declaredFields.mapTo(this) { it.type }
            V1_20_R3RuntimeVersionModuleFactory::class.java.declaredConstructors.flatMapTo(this) { constructor ->
                constructor.parameterTypes.toList()
            }
            V1_20_R3RuntimeVersionModuleFactory::class.java.declaredMethods.flatMapTo(this) { method ->
    ...
    @Test
    fun `matches only the exact v1_20_R3 runtime key`() {
        val factory = V1_20_R3RuntimeVersionModuleFactory()

        assertEquals(true, factory.isCompatible(runtimeVersion("1.20.4", "v1_20_R3")))
        assertEquals(false, factory.isCompatible(runtimeVersion("1.20.5", "v1_20_R4")))
        assertEquals(false, factory.isCompatible(RuntimeServerVersion("test", "1.20.4", null, null)))
    }

    @Test
    fun `uses direct packets for all supported visual features`() {
        val serverVersion = runtimeVersion("1.20.4", "v1_20_R3")
        val module = V1_20_R3RuntimeVersionModuleFactory().create(fakeServer(), serverVersion)

        assertEquals("nms-v1_20_R3", module.id)
        assertEquals("nms-v1_20_R3 with direct titles/actionbar/player-list/sidebar (1.20.4 / v1_20_R3)", module.displayName)
        assertEquals(RuntimeCapabilityStatus.AVAILABLE, module.capabilities.single { it.name == RuntimeCapability.TITLES }.status)
        assertEquals(
            "direct v1_20_R3 title, subtitle, and timing packets",
        ...
        assertEquals(
            "direct v1_20_R3 ClientboundSetActionBarTextPacket",
        ...
        assertEquals(
            "direct v1_20_R3 ClientboundTabListPacket",
        ...
        assertEquals(
            "direct v1_20_R3 scoreboard packets",
    ...
    fun `direct packet methods fall back when player is not CraftPlayer`() {
        ...
        val module = V1_20_R3RuntimeVersionModule(delegate, runtimeVersion("1.20.4", "v1_20_R3"))
    ...
    fun `direct packet sink sends title actionbar and player-list packets`() {
        ...
        val packetSink = V1_20_R3RuntimePacketSink(packets::add)
    ...
    fun `direct sidebar sends objective team score and cleanup packets`() {
        ...
        val packets = mutableListOf<Packet<*>>()
        val player = fakePlayer()
        val sidebar = V1_20_R3RuntimeSidebar(player, packets::add, initialTitle = "Initial")
