package dev.tarkan.titlemanager.nms.legacy.v1_8_R1
...
object LegacyV1_8_R1RuntimeVersionModulePacketSink : LegacyDirectNmsPacketSink {
    override fun sendTitleTimes(player: Player, ticks: LegacyTitleTicks) {
        sendPacket(player, PacketPlayOutTitle(EnumTitleAction.TIMES, null, ticks.fadeIn, ticks.stay, ticks.fadeOut))
    }

    override fun sendTitle(player: Player, title: String, ticks: LegacyTitleTicks) {
        sendPacket(player, PacketPlayOutTitle(EnumTitleAction.TITLE, component(title), ticks.fadeIn, ticks.stay, ticks.fadeOut))
    }

    override fun sendSubtitle(player: Player, subtitle: String, ticks: LegacyTitleTicks) {
        sendPacket(player, PacketPlayOutTitle(EnumTitleAction.SUBTITLE, component(subtitle), ticks.fadeIn, ticks.stay, ticks.fadeOut))
    }
    ...
    private fun component(text: String): IChatBaseComponent = ChatSerializer.a(legacyComponentJson(text))
